$(document).ready(function () {
    const menu = $('.menu');
    const menuItem1 = $('.m-item-1');
    const menuItem2 = $('.m-item-2');
  
    let menuItemHeight = 0;
    let isAnimating = false;
    let closeTimeout;
    let isOpen = false;
  
    // Detect if the device is mobile (touch, no hover)
    const isMobile = window.matchMedia("(hover: none) and (pointer: coarse)").matches;
  
    // Initial CSS state
    menuItem1.css({ display: 'none', zIndex: '-1', maxHeight: '0' });
    menuItem2.css({ display: 'none', zIndex: '-1', maxHeight: '0' });
  
    // Calculate the full height of menu items for animation
    function updateHeights() {
      const tempClone = menuItem1.clone()
        .css({
          position: 'absolute',
          visibility: 'hidden',
          display: 'block',
          maxHeight: 'none',
          zIndex: '-9999'
        })
        .appendTo(menu);
  
      menuItemHeight = tempClone.outerHeight();
      tempClone.remove();
    }
  
    updateHeights();
    $(window).on('resize', updateHeights);
  
    // Common open animation
    function openMenu() {
      if (isAnimating || isOpen) return;
      isAnimating = true;
  
      menuItem1.css({
        zIndex: '-1',
        display: 'block',
        maxHeight: '0',
        overflow: 'hidden'
      });
  
      setTimeout(() => {
        menuItem1.css('z-index', '2')
          .animate({ maxHeight: menuItemHeight }, 150, () => {
            menuItem2.css({
              top: `calc(100% + 24px + ${menuItemHeight}px - 1px)`,
              zIndex: '-1',
              display: 'block',
              maxHeight: '0',
              overflow: 'hidden'
            });
  
            setTimeout(() => {
              menuItem2.css('z-index', '2')
                .animate({ maxHeight: menuItemHeight }, 150, () => {
                  isAnimating = false;
                  isOpen = true;
                });
            }, 50);
          });
      }, 50);
    }
  
    // Common close animation
    function closeMenu() {
      if (isAnimating || !isOpen) return;
      isAnimating = true;
  
      menuItem2.animate({ maxHeight: '0' }, 150, () => {
        menuItem2.css({ display: 'none', zIndex: '-1' });
  
        menuItem1.animate({ maxHeight: '0' }, 150, () => {
          menuItem1.css({ display: 'none', zIndex: '-1' });
          isAnimating = false;
          isOpen = false;
        });
      });
    }
  
    // Desktop: hover trigger
    if (!isMobile) {
      menu.on('mouseenter', () => {
        clearTimeout(closeTimeout);
        openMenu();
      });
  
      menu.on('mouseleave', () => {
        clearTimeout(closeTimeout);
        closeTimeout = setTimeout(closeMenu, 150); // small delay on exit
      });
    } else {
      // Mobile: tap to toggle
      menu.on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        isOpen ? closeMenu() : openMenu();
      });
  
      // Optional: tap outside to close
      $(document).on('click touchstart', function (e) {
        if (!$(e.target).closest('.menu').length && isOpen && !isAnimating) {
          closeMenu();
        }
      });
    }
  });
  
  
  